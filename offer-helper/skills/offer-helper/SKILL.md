...
4. 用 `templates/resume.html` 生成简历：按人群选模块顺序（应届教育在前/社招
   ...
   「数据存放与运行时」规则落盘与命名。
5. 按 `resume/self-check.md` 逐项自检，任一项不过先回改；通过后输出「匹配度
   报告」：命中了 JD 哪些要求、缺口在哪（高权重缺口放显著位置）、建议如何补、
   并附自检勾选结果；再附一句导出提示：「在浏览器打开该 HTML → 打印 → 目标选
   『另存为 PDF』，纸张 A4、边距默认、勾选『背景图形』」（详见
   `references/export-guide.md`）。无 JD 时也可基于经历库生成一份通用简历。
